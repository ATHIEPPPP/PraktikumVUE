  ...
  <div class="bg-white rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100 p-8 max-w-2xl mx-auto">
    <div class="flex items-start justify-between mb-6">
      <div class="flex items-start gap-4">
        ...
        <div>
          <h3 class="text-2xl font-bold text-slate-900 tracking-tight mb-1">Keranjang Pesanan</h3>
    ...
    >
      ...
      <p class="text-slate-900 font-bold text-lg mb-2">Keranjang masih kosong</p>
      ...
      <transition-group name="list">
        ...
        >
          ...
          <div class="flex-1 min-w-0">
            ...
            <p class="text-xs text-slate-500 mb-2 font-medium">🏜️ {{ item.category }}</p>
            <p class="font-bold text-amber-700 text-lg">{{ formatPrice(item.price) }}</p>
          </div>
          ...
          <div class="flex flex-col items-end justify-between">
            ...
            <div class="flex items-center gap-1 bg-white border-2 border-slate-200 rounded-xl shadow-sm">
              <button
              ...
              <span class="w-10 text-center font-bold text-base">{{ item.quantity }}</span>
              <button
                ...
                :class="{ 'opacity-30 cursor-not-allowed': item.quantity >= 99 }"
              >
                +
              </button>
            </div>
    ...
    <!-- Summary -->
    <div v-if="cart.length > 0" class="border-t-2 border-slate-200 pt-6 space-y-4">
      ...
      <div class="pt-2">
        ...
        <transition name="message">
          <p v-if="promoMessage" class="text-xs mt-2 font-medium flex items-center gap-1" :class="promoSuccess ? 'text-green-600' : 'text-red-600'">
      ...
      <transition name="discount">
        <div v-if="discount > 0" class="flex justify-between text-sm p-3 bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl border-2 border-green-200">
          <span class="font-bold text-green-700">🎉 Diskon</span>
          <span class="font-bold text-green-700">-{{ formatPrice(discount) }}</span>
        </div>
      </transition>

      <div class="flex justify-between text-xl font-bold pt-4 mt-4 p-4 bg-gradient-to-r from-amber-50 to-orange-50 rounded-xl border-2 border-amber-200">
        ...
        <div class="bg-white rounded-3xl p-8 max-w-2xl w-full max-h-[80vh] overflow-y-auto shadow-2xl">
          <div class="flex items-center justify-between mb-6">
            <div class="flex items-center gap-3">
              ...
              <h4 class="text-2xl font-bold text-slate-900 tracking-tight">Pilih Menu</h4>
            </div>
            <button
          ...
          <div class="grid gap-4 md:grid-cols-2">
            ...
            >
              ...
              />
              <div class="flex-1 min-w-0">
                <h5 class="font-bold text-sm text-slate-900 truncate mb-1">{{ menu.name }}</h5>
                <p class="text-xs text-slate-500 mb-2 font-medium">{{ menu.category }}</p>
                <p class="font-bold text-amber-700 text-base">{{ formatPrice(menu.price) }}</p>
              </div>
            </button>
          </div>
        </div>
...
const availableMenus = [
  ...
  },
  {
    ...
    category: 'Main Course',
    price: 30000,
    image: 'https://images.unsplash.com/photo-1604908176997-1251884b08a2?auto=format&fit=crop&w=300&q=80',
  },
  {
    id: 3,
  ...
  },
  {
    ...
    category: 'Main Course',
    price: 35000,
    image: 'https://images.unsplash.com/photo-1625937286074-9ca519d5d9df?auto=format&fit=crop&w=300&q=80',
  },
];
...
const applyPromo = () => {
  ...
  if (promoCodes.hasOwnProperty(code)) {
    ...
    if (promoValue <= 100) {
      ...
      promoMessage.value = `Selamat! Diskon ${formatPrice(promoValue)} berhasil diterapkan`;
    }
